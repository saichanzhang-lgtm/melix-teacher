...
if [ -f "$SCRIPT_DIR/Dockerfile" ]; then
  ...
  cp "$SCRIPT_DIR/docker-compose.yml" "$DEPLOY_DIR/" 2>/dev/null || cp "$PROJECT_DIR/docker-compose.yml" "$DEPLOY_DIR/" 2>/dev/null || true
  cp -r "$PROJECT_DIR/web" "$DEPLOY_DIR/" 2>/dev/null || true
fi
